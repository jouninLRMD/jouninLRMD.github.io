...
## Identificar código <tt>R</tt>, <tt>Python</tt> y salidas
...
Por su parte, para el código presentado en <tt>Python</tt> se empleará un fondo de color rojo tal como se presenta a continuación.

```{python eval=FALSE}
'Ejemplo código Python'
```

Mientas que, para los resultados o salidas obtenidas tanto en <tt>R</tt> como en <tt>Python</tt> se empleará un fondo de color gris, tal como se presenta a continuación.
...
datos2 <- cbind(
  ...
  "Operador en R" = c("A+B", "A-B", "A*B", "A%*%B", "t(A)", "diag(A)", "solve(A)", "det(A)"),
